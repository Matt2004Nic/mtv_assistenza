...
async def main(event):
    ...
    if len(message)==0 and media!=None and chat_id != 1881287074:
        if username != None:  
            commentoSussyBaka1 = f"Messaggio inviato da (@{username})"
        elif phone != None:
            commentoSussyBaka1= f"Messaggio inviato da (+{phone}) (#{chat_id})"
        else:
            commentoSussyBaka1 = f"Messaggio inviato da ({first_name}) (#{chat_id})"
        await client.send_file(1881287074,media,caption = commentoSussyBaka1)
    if chat_id != 1881287074 and controllo:
        if media != None:
            await client.send_file(1881287074,media,caption = commentoSussyBaka)
        else:
        ...
        if len(message_text_c)>0:
            ...
            if media != None and len(message)>0:
                await client.send_file(message_text_c[0],media,caption = message)
            elif media!= None and len(message)==0:
                await client.send_file(message_text_c[0],media)
            else:
            ...
            if len(message_text_cancelletto)>0:
                ...
                tag = tag.replace(')','')
                if media != None and len(message)>0:
                    await client.send_file(int(tag),media,caption = message)
                elif media!= None and len(message)==0:
                    await client.send_file(int(tag),media)
                else:
